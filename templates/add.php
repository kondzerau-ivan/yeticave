    ...
    <form class="form form--add-lot container <?= !empty($errors) ? 'form--invalid' : ''; ?>" action="/add.php" method="post" enctype="multipart/form-data">
        ...
        <div class="form__container-two">
            <div class="form__item<?= isset($errors['lot-name']) ? ' form__item--invalid' : ''; ?>">
                ...
                <span class="form__error"><?= isset($errors) ? renderErrorsMessage($errors, 'lot-name') : ''; ?></span>
            </div>
            <div class="form__item<?= isset($errors['category']) ? ' form__item--invalid' : ''; ?>">
                ...
                <select id="category" name="category">
                    ...
                    <?php foreach ($categories as $category) : ?>
                        <option value="<?= htmlspecialchars($category['code']); ?>" <?= getPostVal('category') == $category['code'] ? 'selected' : ''; ?>><?= htmlspecialchars($category['name']); ?></option>
                    <?php endforeach; ?>
                </select>
                <span class="form__error"><?= isset($errors) ? renderErrorsMessage($errors, 'category') : ''; ?></span>
            </div>
        </div>
        ...
        <div class="form__item form__item--file<?= isset($errors['image']) ? ' form__item--invalid' : ''; ?>">
            ...
            <span class="form__error"><?= isset($errors) ? renderErrorsMessage($errors, 'image') : ''; ?></span>
        </div>
        <div class="form__container-three">
            <div class="form__item form__item--small<?= isset($errors['lot-rate']) ? ' form__item--invalid' : ''; ?>">
                <label for="lot-rate">Начальная цена <sup>*</sup></label>
                <input id="lot-rate" type="text" name="lot-rate" placeholder="0" value="<?= getPostVal('lot-rate'); ?>">
                <span class="form__error"><?= isset($errors) ? renderErrorsMessage($errors, 'lot-rate') : ''; ?></span>
            </div>
            <div class="form__item form__item--small<?= isset($errors['lot-step']) ? ' form__item--invalid' : ''; ?>">
                <label for="lot-step">Шаг ставки <sup>*</sup></label>
                <input id="lot-step" type="text" name="lot-step" placeholder="0" value="<?= getPostVal('lot-step'); ?>">
                <span class="form__error"><?= isset($errors) ? renderErrorsMessage($errors, 'lot-step') : ''; ?></span>
            </div>
            <div class="form__item<?= isset($errors['lot-date']) ? ' form__item--invalid' : ''; ?>">
                ...
                <span class="form__error"><?= isset($errors) ? renderErrorsMessage($errors, 'lot-date') : ''; ?></span>
            </div>
        </div>
